...
| `rubro_c` | Rubro__c | Lookup (tabla Rubro) | **[NUEVO] Debe ser el Id** de un registro de la tabla Rubro__c |
| `status_description` | PSE_Descripcion_de_Estado__c | Texto | Longitud maxima de 255 |

### Valores de Picklists y tablas relacionadas

#### [NUEVO] Modelo de datos: Tipo de Negocio, Rubro y Subrubro

Actualmente la clasificación funciona con **tres niveles** relacionados en Salesforce:

- **Tipo de Negocio**
  - Campo picklist en el objeto Lead (`Tipo_de_Negocio__c` / `Tipo de Negocio` en el formulario).
  - Valores de ejemplo: `Equipos`, `Repuestos y Consumibles`, etc.
- **Rubro__c (Rubro)**
  - Es una **tabla/objeto** (`Rubro__c`).
  - Tiene al menos:
    - `Name`: nombre legible del rubro (por ejemplo `"Maquinaria CAT"`).
    - `Tipo_de_Negocio__c`: picklist que indica a qué **Tipo de Negocio** pertenece ese rubro.
- **Subrubro__c (Subrubro)**
  - Es otra **tabla/objeto** (`Subrubro__c`).
  - Tiene al menos:
    - `Name`: nombre legible del subrubro (por ejemplo `"Usado CAT"`).
    - `Rubro__c`: lookup que apunta al **Rubro__c** padre.

#### [NUEVO] Comportamiento en el formulario Web-to-Lead

1. El usuario selecciona un **Tipo de Negocio** en el formulario.
2. El frontend hace un **fetch** a la API y filtra los registros de `Rubro__c` para mostrar solo los rubros cuyo campo `Tipo_de_Negocio__c` coincide con el Tipo de Negocio elegido.
3. Cuando el usuario selecciona un **Rubro**, se filtran los registros de `Subrubro__c` para mostrar solo los subrubros cuyo lookup `Rubro__c` apunte al rubro seleccionado.
4. Al enviar el formulario, hacia Salesforce se envían los **Ids** de `Rubro__c` y `Subrubro__c` seleccionados, no los nombres (`Name`).

#### Lista histórica de Rubros (solo referencia)

> Nota: La siguiente lista se mantiene solo como referencia histórica de valores.  
> Hoy en día estos valores viven como **registros de la tabla** `Rubro__c` y se consultan dinámicamente desde el frontend.
...
- **[NUEVO] Actualizado**: Se actualizó el formulario de ejemplo Web-to-Lead con la nueva versión generada desde Salesforce, incluyendo el nuevo picklist `Tipo de Negocio` (`Equipos`, `Repuestos y Consumibles`).
...
Tipo de Negocio:<select  id="00NOx00000gLrnV" name="00NOx00000gLrnV" title="Tipo de Negocio"><option value="">--None--</option><option value="Equipos">Equipos</option>
<option value="Repuestos y Consumibles">Repuestos y Consumibles</option>
</select><br>
...
### 2026-03-10
- **[NUEVO] Cambio de modelo de datos**: Los campos `Rubro__c`, `Subrubro__c` y `Division__c` ahora se basan en **tablas (objetos)** dentro de Salesforce.  
  - **[NUEVO]** En los formularios Web-to-Lead se hace un **fetch** a la API para obtener los registros y poblar los combos de selección.  
  - **[NUEVO]** En los payloads enviados a Salesforce (API REST y Web-to-Lead) se debe enviar **el Id del registro** (por ejemplo `a01XXXXXXXXXXXXXXX`) en lugar del nombre legible como `"Maquinaria CAT"` o `"Nuevo CAT"`.