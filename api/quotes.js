...
async function getSalesforceToken(environment) {
  ...
  const assertion = jwt.sign({ iss: clientId, sub: username, aud: tokenUrl.includes('test.salesforce.com') ? 'https://test.salesforce.com' : 'https://login.salesforce.com', exp: Math.floor(Date.now() / 1000) + 300, sc: 'api' }, privateKey, { algorithm: 'RS256' });
