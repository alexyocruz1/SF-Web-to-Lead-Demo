...
function buildSalesforceQueryUrl(instanceUrl, apiVersion, soql) {
  if (!instanceUrl || !apiVersion || !soql) return null;
  return `${instanceUrl}/services/data/v${apiVersion}/query?q=${encodeURIComponent(soql)}`;
}

function getSuggestedUserEndpoint(environment) {
  const useProd =
  ...
  if (!instance || !apiVersion) return null;
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const soql = 'SELECT Id, Name FROM User WHERE IsActive = true ORDER BY Name LIMIT 200';

    const userRecords = await querySalesforce(
      access_token,
      instance_url,
      apiVersion,
      soql
    ...
    const users = userRecords.map(u => ({ id: u.Id, name: u.Name }));

    return res.status(200).json({
      endpoint: getSuggestedUserEndpoint(environment),
      soql,
      queryUrl: buildSalesforceQueryUrl(instance_url, apiVersion, soql),
      users,
    });
  } catch (error) {
