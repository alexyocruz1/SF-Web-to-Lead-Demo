...
async function getSalesforceToken(environment) {
  ...
  let privateKey;
  if (privateKeyOrPath.includes('BEGIN PRIVATE KEY') || privateKeyOrPath.includes('BEGIN RSA PRIVATE KEY')) privateKey = privateKeyOrPath.replace(/\\n/g, '\n').trim();
  else privateKey = fs.readFileSync(path.isAbsolute(privateKeyOrPath) ? privateKeyOrPath : path.join(process.cwd(), privateKeyOrPath), 'utf8').trim();
  const assertion = jwt.sign({ iss: clientId, sub: username, aud: tokenUrl.includes('test.salesforce.com') ? 'https://test.salesforce.com' : 'https://login.salesforce.com', exp: Math.floor(Date.now() / 1000) + 300 }, privateKey, { algorithm: 'RS256' });
  const res = await fetch(tokenUrl, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ grant_type: 'urn:ietf:params:oauth:grant-type:jwt-bearer', assertion }).toString() });
...
function getSuggestedEndpoints(environment) {
  ...
  const localEndpoint = `/api/opportunitylineitems?environment=${environment}`;
  if (!instance) return { create: localEndpoint, localEndpoint };
  return { create: `https://${instance}/services/data/v${apiVersion}/sobjects/OpportunityLineItem`, localEndpoint };
}
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const proxyRes = await fetch(actualEndpoint, { method, headers, body: method === 'GET' ? undefined : JSON.stringify(body || {}) });
